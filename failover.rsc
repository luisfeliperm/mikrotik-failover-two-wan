...
:local x 1
:do {
	# :log info "[ScriptFailOver.Retry.$x] Iniciando retry $x"
	...
	if ($totalAcceptPing>0) do={
		...
		if ($totalAcceptPing>0) do={
			...
			# :log info "[ScriptFailOver.Retry.$x] Falhou nos dois target"
			if ($retry = $x) do={
				# :log info "[ScriptFailOver.Retry.$x] Tentativas esgotadas"
